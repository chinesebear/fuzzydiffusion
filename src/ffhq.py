...
def ffhq_train(config_path, delegate_path, rule_num):
    setup_seed(10)
    batch_size = 32
    img_shape = (3,256,256)
    z_shape = (4,32,32)
    ...
    root_path = options.base_path+f'output/log/ffhq/'+now +'/'
    ...
    logger.info("ffhq test start")
    ffhq_test(fldmodel, root_path)
    logger.info("ffhq test done")
    logger.remove(log_file)

def ffhq_test(fldmodel, root_path):
    fldmodel.eval()
    frozen_model(fldmodel)
    log_file = logger.add(root_path+'fuzzy-latent-diffusion-test-'+str(datetime.date.today()) +'.log')
    metrics_csv_path = root_path+f'ffhq_{fldmodel.rule_num}rules_metrics-'+str(datetime.date.today()) +'.csv'
...
if __name__ == '__main__':
    ffhq_train("/home/yang/sda/github/fuzzydiffusion/src/config/latent-diffusion/ffhq-ldm-vq-4.yaml",
               "/home/yang/sda/github/fuzzydiffusion/output/delegates/ffhq/ffhq_3_delegates.csv",
               3)