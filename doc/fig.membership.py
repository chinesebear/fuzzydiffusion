...
df2=pd.read_csv("doc/csv/dfs_sampled_50.csv")
x= np.flipud(df2['x'].values)
xlen = len(x)
landspace2=np.flipud(df2['y1'].values)
animal2=np.flipud(df2['y2'].values)
human2=np.flipud(df2['y3'].values)
df3=pd.read_csv("doc/csv/noalig_line_sampled_50.csv")
landspace3=np.flipud(df3['y1'].values)
animal3=np.flipud(df3['y2'].values)
human3=np.flipud(df3['y3'].values)
...
# plt.grid()
plt.legend()
plt.text(25, -0.2, "(a)",fontsize=18, color='black')
...
# plt.grid()
plt.legend()
plt.text(25, -0.2, "(b)",fontsize=18, color='black')
plt.gca().invert_xaxis()



fig.tight_layout()#调整整体空白
plt.savefig("doc/fig.membership.svg", format = "svg",  transparent=True,dpi=600)
plt.savefig("doc/fig.membership.jpg", format = "jpg",  transparent=True,dpi=600)